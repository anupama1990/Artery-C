...
void InetRadioDriver::initialize(int stage)
{
    if (stage == inet::INITSTAGE_LOCAL) {
        ...
        std::call_once(register_protocol_flag, []() {
            inet::ProtocolGroup::ethertype.addProtocol(0x8947, &vanetza::geonet);
        });
...
void InetRadioDriver::handleDataRequest(cMessage* msg)
{
    ...
    proto_tag->setProtocol(&vanetza::geonet);
    assert(request->ether_type.host() == inet::ethertype.findProtocolNumber(&vanetza::geonet));
